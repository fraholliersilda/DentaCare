document.addEventListener("DOMContentLoaded", function () {
  ...
  function validatePhone(phone) {
    ...
    return (
      albanianMobilePattern.test(phone) ||
  ...
  function validateDate(date) {
    ...
    if (selectedDate > maxDate) {
      return {
        valid: false,
        message: "Data nuk mund të jetë më shumë se 6 muaj në të ardhmen",
      };
    }
  ...
  function validateTime(time, date) {
    ...
    if (selectedDate.toDateString() === today.toDateString()) {
      ...
      if (selectedTime <= currentTime + 60) {
        return {
          valid: false,
          message: "Ju lutemi zgjidhni një orë të paktën 1 orë në të ardhmen",
        };
      }
    }
  ...
  function validateTextLength(text, maxLength, fieldName) {
    if (text && text.length > maxLength) {
      return {
        valid: false,
        message: `${fieldName} nuk mund të ketë më shumë se ${maxLength} karaktere`,
      };
    }
    return { valid: true };
  }
  ...
  function showServiceSelectionError() {
    ...
    const existingError = serviceSelection.parentNode.querySelector(
    ...
    serviceSelection.parentNode.insertBefore(
      errorDiv,
      serviceSelection.nextSibling
    );
  }
  ...
  document.getElementById("fullName").addEventListener("blur", function () {
    ...
    if (!value) {
      ...
      showFieldError(
        "fullName",
        "Emri dhe mbiemri duhet të kenë të paktën 2 karaktere"
      );
    } else {
  ...
  document
    ...
    .addEventListener("submit", function (e) {
      ...
      const problemLengthValidation = validateTextLength(
        problem,
        500,
        "Përshkrimi i problemit"
      );
      ...
      if (isValid) {
        ...
        document
          .querySelectorAll(".service-error-message")
          .forEach((error) => error.remove());
        document
